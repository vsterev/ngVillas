...
module.exports = {
  get: {
    ...
    villaDetails: (req, res, next) => {
      ...
      villaModel
        .findById(villaId)
        .populate('creatorId reservationId')
        .then((villa) => {
  ...
  post: {
    book: (req, res, next) => {
      ...
      reservationModel
        .create({ clients, villaId, creatorId: user.id, comments: [comment] })
        .then((reservation) => {
          Promise.all([
            reservation,
    ...
    edit: (req, res, next) => {
      ...
      villaModel
        ...
        .then((villa) => {
          ...
          return villaModel.findByIdAndUpdate(
            villaId,
            {
              name,
              region,
    ...
    allVillasExtended: (req, res, next) => {
      ...
      if (search) {
        query = {
          ...query,
          $or: [{ name: { $regex: search, $options: 'i' } }, { region: { $regex: search, $options: 'i' } }],
        };
      }
      if (to) {
